...
final class Asset extends Component {
	...
	public function add_image_sizes() {
		foreach ( stmotheme()->get_config( 'image_sizes' ) as $name => $args ) {
			add_image_size( st\prefix( $name ), $args['width'], st\get_array_value( $args, 'height', 0 ), st\get_array_value( $args, 'crop', false ) );
		}
	}
	...
	public function enqueue_styles() {
		...
		// Get styles.
		$styles = stmotheme()->get_config( 'styles' );

		// Filter styles.
		$styles = array_filter(
			$styles,
			function( $style ) {
				$scope = (array) st\get_array_value( $style, 'scope' );
		...
		foreach ( $styles as $style ) {
			wp_enqueue_style( $style['handle'], $style['src'], st\get_array_value( $style, 'deps', [] ), st\get_array_value( $style, 'version', stmotheme()->get_version() ) );
		}
	}
	...
	public function enqueue_editor_styles() {
		foreach ( stmotheme()->get_config( 'styles' ) as $style ) {
			if ( in_array( 'editor', (array) st\get_array_value( $style, 'scope' ), true ) ) {
	...
	public function enqueue_scripts() {
		...
		// Get scripts.
		$scripts = stmotheme()->get_config( 'scripts' );

		// Filter scripts.
		$scripts = array_filter(
			$scripts,
			function( $script ) {
				$scope = (array) st\get_array_value( $script, 'scope' );
		...
		foreach ( $scripts as $script ) {
			wp_enqueue_script( $script['handle'], $script['src'], st\get_array_value( $script, 'deps', [] ), st\get_array_value( $script, 'version', stmotheme()->get_version() ), st\get_array_value( $script, 'in_footer', true ) );
	...
	public function wrap_embeds( $html, $url, $attr, $post_id ) {
		if ( ! has_blocks( $post_id ) ) {
			...
			// Get embed size.
			$size = st\get_last_array_value( $matches );

			if ( is_array( $size ) && count( $size ) === 2 ) {
				$width  = absint( st\get_first_array_value( $size ) );
				$height = absint( st\get_last_array_value( $size ) );

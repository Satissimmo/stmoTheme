...
final class Customizer extends Component {
	...
	protected function get_theme_styles() {
		...
		if ( false === $styles || is_customize_preview() ) {
			...
			foreach ( stmotheme()->get_config( 'theme_styles' ) as $style ) {
					...
					if ( $value ) {
						switch ( $property['name'] ) {
							...
							case 'font-family':
								...
								$value = st\get_first_array_value( explode( ':', $value ) ) . ', sans-serif';
	...
	public function set_theme_defaults() {
		foreach ( stmotheme()->get_config( 'theme_mods' ) as $section ) {
	...
	public function register_theme_mods( $wp_customize ) {
		foreach ( stmotheme()->get_config( 'theme_mods' ) as $section_name => $section ) {
			...
			if ( ! in_array( $section_name, array_keys( $wp_customize->sections() ), true ) ) {
				$wp_customize->add_section(
					...
					[
						'title'       => $section['title'],
						'description' => st\get_array_value( $section, 'description' ),
						'priority'    => st\get_array_value( $section, 'priority' ),
					]
				);
			}
			...
			foreach ( $section['fields'] as $field_name => $field ) {
				...
				$wp_customize->add_setting(
					$field_name,
					[
						'default'           => st\get_array_value( $field, 'default' ),
				...
				switch ( $field['type'] ) {
					...
					default:
						...
						if ( 'font' === $control['type'] ) {
							$control['type']    = 'select';
							$control['choices'] = stmotheme()->get_config( 'fonts' );
						}
	...
	public function request_theme_styles( $response, $args, $url ) {
		if ( strpos( $url, 'https://stmotheme-editor-css' ) === 0 ) {
			$response = [
	...
	public function add_theme_styles() {
		wp_add_inline_style( 'stmotheme-parent-frontend', $this->get_theme_styles() );
	}
	...
	public function add_editor_styles() {
		...
		// Enqueue styles.
		add_editor_style( 'https://stmotheme-editor-css' );
